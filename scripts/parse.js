...
export async function parse(rawContent, options = {}) {
  ...
  if (trimmed.startsWith('---')) {
    ...
    if (!match) throw new Error(`${sourceName}: frontmatter block is not closed correctly.`);
    frontmatter = yaml.load(match[1]) || {};
    body = match[2].trim();
  } else {
    const legacy = parseLegacyFrontmatter(trimmed);
    if (legacy) {
  ...
  if (!title) throw new Error(`${sourceName}: missing required frontmatter "title".`);

  const slug = slugify(frontmatter.slug || title);
  if (!slug || slug === 'untitled') throw new Error(`${sourceName}: refused to publish with unsafe slug "${slug || 'empty'}".`);
...
function parseLegacyFrontmatter(raw) {
...
function normalizeCategory(category) {
  ...
  return allowedCategories.has(normalized) ? normalized : 'notes';
}
...
function normalizeDate(date) {
  ...
  if (Number.isNaN(parsed.getTime())) throw new Error(`Invalid date "${date}". Use YYYY-MM-DD.`);
...
function normalizeDescription(description, body) {
  ...
  if (!text) throw new Error('Missing required frontmatter "description" and body is empty.');
